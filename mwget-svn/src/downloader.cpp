...
Downloader::init_local_file_name(void)
{
	...
	localMg = tmpStr;
	//printf("%s\n",localPath);
	//publiclocalPath=localPath;
	//fail_record_registry_file(localPath);
//	printf("task.url.get_url()=%s",task.url.get_url());
	//surltemp=task.url.get_url();
	fail_record_registry(localPath,task.url.get_url());//记录
	return 0;
};
